...
def play_song(channel_index, song_index):
    ...
    paused[channel_index] = False
    paused_duration[channel_index] = 0
...
def draw_menu():
    ...
    for j in range(2):
        bar_x = 50 + j * 350
        bar_y = y_offset + len(songs) * 40 + 30 + 40
        if channels[j].get_busy() and start_times[j] is not None:
            ...
            screen.blit(song_name_surf, (bar_x, bar_y - 40))

            # Calculate and draw the progress bar
            if paused[j] and pause_start_times[j] is not None:
                # Ensure that paused duration is not updated while paused
                current_paused_duration = paused_duration[j] + (time.time() - pause_start_times[j])
            else:
                current_paused_duration = paused_duration[j]

            elapsed_time = (time.time() - start_times[j]) - current_paused_duration
            progress = max(0, min(1, elapsed_time / song_durations[j]))
...
def left_right_cb(channel):
    global ch1_pause, pause_start_times, paused_duration
    if ch1_pause:
        channels[0].unpause()
        ch1_pause = False
        paused_duration[0] += time.time() - pause_start_times[0]
        pause_start_times[0] = None  # Reset pause start time
    else:
        channels[0].pause()
        ch1_pause = True
        pause_start_times[0] = time.time()  # Set pause start time

def right_left_cb(channel):
    global ch2_pause, pause_start_times, paused_duration
    if ch2_pause:
        channels[1].unpause()
        ch2_pause = False
        paused_duration[1] += time.time() - pause_start_times[1]
        pause_start_times[1] = None  # Reset pause start time
    else:
        channels[1].pause()
        ch2_pause = True
        pause_start_times[1] = time.time()  # Set pause start time
