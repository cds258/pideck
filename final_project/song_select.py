...
music_directory = 'audio_files'  # Change this to your folder path
...
try:
    while True:
        if not GPIO.input(17):  # Select button
            play_song(current_index)
            time.sleep(0.5)  # Debounce delay

        if not GPIO.input(22):  # Next song
            current_index = (current_index + 1) % len(songs)
            draw_menu()
            time.sleep(0.5)  # Debounce delay

        if not GPIO.input(23):  # Previous song
            current_index = (current_index - 1) % len(songs)
            draw_menu()
            time.sleep(0.5)  # Debounce delay
