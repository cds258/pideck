...
def right_inc_volume(channel):
  global vr
  vr *= 1.1

def right_dec_volume(channel):
  global vr
  vr /= 1.1

def left_ch_volume(channel):
  global vr
  if vl < 2:
    vl *= 1.1
  else:
...
GPIO.add_event_detect(5, GPIO.BOTH, callback=right_inc_volume, bouncetime=200)
GPIO.add_event_detect(4, GPIO.BOTH, callback=right_dec_volume, bouncetime=200)
GPIO.add_event_detect(18, GPIO.BOTH, callback=left_ch_volume, bouncetime=200)
...
def update_volumes():
    ...
    left_volume = vl * data['pot#l'] * data['pot#c']  # Increases as pot#c goes from 0 to 1
    right_volume = vr * data['pot#r'] * (1 - data['pot#c'])  # Decreases as pot#c goes from 0 to 1
