...
def load_waveforms():
    for song in songs:
        rate, data = wavfile.read(os.path.join(music_directory, song))
        if data.ndim > 1:
            data = data.mean(axis=1)
        waveforms.append((rate, np.interp(data, (data.min(), data.max()), (0, 100))))
...
# Attach GPIO callbacks and loop
# (Assume GPIO callback functions are defined here similarly to previous example)
